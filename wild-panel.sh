...
# MmD
#
# Wild Panel management menu: the `wild-panel` command.
#
# Installed to /usr/bin/wild-panel by `wild-panel-amd64 install-menu`, which deploy.sh runs
...
# it branches on comes from `wild-panel-amd64 info --get <field>`, whose field names
...
BIN="${WILDPANEL_BIN:-${VPNUI_BIN:-/opt/wild-panel/wild-panel-amd64}}"
...
# Read one field of `wild-panel-amd64 info --json` by name; prints the raw value.
# Tolerant of failure (empty output) so a caller under `set -e` can branch on it.
info_get() { "$BIN" info --get "$1" 2>/dev/null || true; }

# The panel's systemd unit. NEVER hardcode "wild-panel": the name is operator
...
# (setsid ./wild-panel-amd64 &) with the unit inactive-but-enabled. systemd then
...
say_unmanaged() {
    ...
    act  "it was started outside systemd (e.g. ${TEAL}setsid ./wild-panel-amd64 &${R}), so systemd cannot manage it."
}
...
# 2) Un-Install. The binary prompts for confirmation and removes /usr/bin/wild-panel
...
show_menu() {
    ...
    printf '\n'
    hr
    printf '%s[%sWild Panel%s]%s management   %sv%s%s\n' \
...
environment:
  WILDPANEL_BIN     path to the panel binary (default: /opt/wild-panel/wild-panel-amd64)
  VPNUI_BIN         legacy alias for WILDPANEL_BIN
...
# Acquire root: re-exec through sudo when not already root, so `wild-panel` just works
...
require_bin() {
    [[ -x "$BIN" ]] || die "panel binary not found at ${BIN}. Set WILDPANEL_BIN=/path/to/wild-panel-amd64 if it lives elsewhere."
}
