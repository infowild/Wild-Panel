...
> [!NOTE]
> نصب تازه از `/opt/wild-panel`، یونیت `wild-panel` و دیتابیس `wild-panel.db` استفاده می‌کند. نصب‌های قدیمی هنگام ارتقا خودکار منتقل می‌شوند. نسخهٔ فعلی پنل **2.0.3** است.
